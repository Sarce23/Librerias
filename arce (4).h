...
void servicio(Tnodo temp)//Servicio dentro de una lista/cola/pila
{
	printf("\n%d \n\n",temp->dato);
	system("PAUSE");
}

void Agregarl(Tnodo *lista, Tnodo *nuevo)//agregar dentro de una lista
{
	Tnodo temp=NULL;
...
Tnodo Buscarl(Tnodo lista, int dato)//buscar dentro de una lista
{
	Tnodo temp=NULL;
...
void Imprimirl(Tnodo lista)//imprimir una lista
{
	Tnodo temp=NULL;
...
Tnodo Eliminarl(Tnodo *lista,int dato)//eliminar datos de una lista 
{
	...
	if(*lista)
	  {
		 ...
		 else
		  {
			...
			if(temp->sig && (temp->sig)->dato == dato)
			  {
			  	nodo=temp->sig;
			  	temp->sig=nodo->sig;
			  	nodo->sig=NULL;
